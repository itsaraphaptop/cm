        ...
        <div class="page-header">
          <div class="page-header-content">
            <div class="page-title">
              <h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold"> Home - Project Cashflow Cash</span></h4>
            </div>
          </div>
          ...
          <div class="breadcrumb-line">
            <ul class="breadcrumb">
              ...
              <li><a href="<?php echo base_url(); ?>management/project_status_overview">Project Cashflow Cash</a></li>
          </div>
        </div>
        ...
        <div class="panel-body">
                  <div class="tabbable">
                    <ul class="nav nav-tabs nav-tabs-top top-divided">
                      <li  class="active"><a href="#top-divided-tab" data-toggle="tab">Project by Cash</a></li>
                      <li id="Calendar_li"><a href="#top-divided-tab1" data-toggle="tab">Calendar</a></li>
                      <li id="tab_Graph"><a href="#top-divided-tab2" data-toggle="tab">Graph</a></li>
                  ...
                  <div class="table-responsive">
                    <table class="table">
                      ...
                      <div class="tab-pane" id="top-divided-tab1">
                          <div id="Cut_off_receipt_calender"></div>
                      </div>

                      <div class="tab-pane" id="top-divided-tab2">
                          <h5 style="display: inline;"> Chart Type : </h5><select id="type_chart"></select>
                          <div id="Graph_Cashflow_Cash" class="mychart" style="height: 500px;">  
...
<script type="text/javascript">
  function showCalender(target_tag,data_date,defaultDate){
    ...
    $(target_tag).fullCalendar({
          ...
          eventClick:  function(event, jsEvent, view) {
             ...
             $("#detail_content").html(duedate+" "+color+" "+url);
             $("#detail_modal").modal("toggle");
          }
      });
  }
  ...
  // http://localhost/cloudmeka/management_active/json_to_calender_Cut_off_receipt
  $(function(){
    ...
    var url_calender = '<?=base_url() ?>management_active/json_to_calender_Cut_off_receipt';
      $.get(url_calender, function() {
        
      }).done(function(data){

            try{
             
              var data_datetime = jQuery.parseJSON(data);
              ...
              $("#Calendar_li").click(function(){
                $("html").append('<div class="loading">Loading&#8230;</div>');
                   ...
                   setTimeout(function(){ 
                    $(".loading").remove();
                      showCalender("#Cut_off_receipt_calender",data_datetime,null);
                        
                    }, 500);
              });
            }catch(e){
                alert("error json => "+url);
            }

      }); // get 


      var url_graph = '<?=base_url() ?>management_active/get_json_Graph_Cashflow_Account';
      $("html").append('<div class="loading">Loading&#8230;</div>');
      $.get(url_graph, function() {
        /*optional stuff to do after success */
      }).done(function(data){
            try{
                var json_graph = jQuery.parseJSON(data);
                  //init chart 
                  $(".loading").remove();
                 $("#tab_Graph").click(function(event) {
                  $("html").append('<div class="loading">Loading&#8230;</div>');
                    setTimeout(function(){ 
                      $(".loading").remove();
                        chart_render("#Graph_Cashflow_Cash","column",0,json_graph.categories,json_graph.data,"กราฟแสดง ตัดจ่าย และ ออกใบเสร็จ",json_graph.project_code)
                    }, 500);
                  });


                  $.each(chart_type, function(index, val) {
                           $("#type_chart").append(`<option value="${val}">${val}</option>`);    
                  });

                  $("#type_chart").change(function(event) {
                       chart_render("#Graph_Cashflow_Cash",$(this).val(),0,json_graph.categories,json_graph.data,"กราฟแสดง ตัดจ่าย และ ออกใบเสร็จ",json_graph.project_code);
                  });
            }catch(e){
              alert( "ajax error => "+url_graph);
