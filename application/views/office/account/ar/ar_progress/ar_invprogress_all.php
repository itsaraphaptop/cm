...
<div class="panel panel-flat">
	<div class="panel-heading">
		<h5 class="panel-title">Invoice Progress Archive</h5>
	...
	<button class="all label bg-orange"> Invoice Progress ALL</button>
	<button class="not label label-danger">Invoice Progress not Receipt</button>
	<button class="to label bg-green">Invoice Progress Receipt</button>
	</div>
	<script>
		$(".all").click(function(){
			$(".loadtable").html("<img src='<?php echo base_url();?>assets/images/loading.gif'> Loading...");
			$(".loadtable").load('<?php echo base_url(); ?>ar/archiveprogress_ar_all');
		});
		$(".not").click(function(){
			$(".loadtable").html("<img src='<?php echo base_url();?>assets/images/loading.gif'> Loading...");
			$(".loadtable").load('<?php echo base_url(); ?>ar/archiveprogress_ar_no');
		});

		$(".to").click(function(){
			$(".loadtable").html("<img src='<?php echo base_url();?>assets/images/loading.gif'> Loading...");
			$(".loadtable").load('<?php echo base_url(); ?>ar/archiveprogress_ar_to');
		});
	</script>
			...
			<table class="table table-hover table-bordered table-xxs datatable-basic">
				<thead >
					<tr>
						...
						<th class="text-center">Invoice Progress</th>
				...
				<tbody class="text-center">
					<?php $n=1; foreach($pro as $e){?>
					<tr>
						<td><?php echo $n; ?></td>
						<?php 
							$de=$this->db->query("SELECT sum(inv_vatamt) as vatamt,sum(inv_lesswt) as inv_wt,sum(inv_progressamt) as toamt from ar_invprogress_detail where inv_ref = '$e->inv_no' and inv_period = $e->inv_period")->result();
							foreach ($de as $dd) {
								$vatamt = $dd->vatamt;
								$wtamt = $dd->inv_wt;
								$amt = $dd->toamt;
						...
						<td>
							<ul class="icons-list">
								...
								<li><a href="<?php echo base_url(); ?>ar/ar_edit_invprogress/<?php echo $e->inv_no; ?>/<?php echo $e->inv_period; ?>" data-popup="tooltip" title=""><i class="icon-pencil7"></i></a></li>
								<li><a href="<?php $base_url = $this->config->item('url_report'); echo $base_url; ?>stimulsoft/Flex/stimulsoft/index.php?stimulsoft_client_key=VieweFx&stimulsoft_report_key=ar_invprogress_report.mrt&id=<?php echo $e->inv_no; ?>&period=<?php echo $e->inv_period; ?>&compcode=<?php echo $compcode;?>" data-popup="tooltip" title="" target="_blank" data-original-title="Print"><i class="icon-printer4"></i></a></li>
							 </ul>
						</td>
					</tr>
					<?php $n++; } ?>
        ...
        <div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-success" id="saveprogress<?php echo $ee->inv_no.$ee->inv_period;?>"><i class="icon-floppy-disk"></i> Save</button>
...
<script>
	...
	$("#saveprogress<?php echo $ee->inv_no.$ee->inv_period;?>").click(function(){
	var url="<?php echo base_url(); ?>ar_active/ar_progressremore";
	var dataSet={
	...
	$.post(url,dataSet,function(data){
        setTimeout(function() {
          window.location.href = "<?php echo base_url(); ?>index.php/ar/open_invprogress_all";
        }, 1000);
      });
	} );
</script>
