									             ...
									             <table class="table table-hover table-bordered  table-xxs datatable-basic">
													...
													<tbody>
															<?php foreach ($getproj as $key => $value) {
																$this->db->select('*');
													         	...
													         	$this->db->where('inv_status',"N");
													          	$q = $this->db->get('ar_invdown_header');
																$result1 = $q->num_rows();

																$this->db->select('*');
													          	$this->db->where('inv_project',$value->project_id);
													         	$this->db->where('inv_status',"N");
													          	$q = $this->db->get('ar_invprogress_header');
													          	$result2 = $q->num_rows();
													          	$this->db->select('*');
													          	$this->db->where('inv_project',$value->project_id);
													         	$this->db->where('inv_status',"N");
													          	$q = $this->db->get('ar_invretention_header');
																$result3 = $q->num_rows();
															...
															<tr>
																...
																<td><a href="<?php echo base_url(); ?>index.php/ar/ar_account/<?php echo $value->project_id; ?>" class="label label-primary"><i class="icon-folder-open"></i> Select</a></td>
            ...
            <div class="modal-dialog modal-lg">
              <div class="modal-content">
                <div class="modal-header bg-primary">
                  ...
                  <h4 class="modal-title" id="myModalLabel">View Project Account</h4>
                </div>
	...
	<div id="sub<?php echo $val->project_code;?>" class="modal fade" data-backdrop="false">
					 <div class="modal-dialog">
	                    ...
	                    <td class="text-center"><?php  if($keysub->controlbg=='2'){ echo '<input type="checkbox" checked="checked" disabled="disabled">';}else{ echo '<input type="checkbox"  disabled="disabled">';} ?></td>
                                <td class="text-center"><a href="<?php echo base_url(); ?>index.php/ar/ar_account/<?php echo $value->project_id; ?>" class="label label-primary"><i class="icon-folder-open"></i> Select</a></td>
