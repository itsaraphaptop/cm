		...
		<div class="panel-body">
			...
			<div class="row form-group">
				<div class="col-xs-2 col-xs-offset-2">
					...
					<input type="hidden" name="status_save" value="s">
				</div>
				<div class="col-xs-3">
				...
				<table class="table table-striped table-bordered table-xxs table-hover" style="overflow-x: auto">
					...
					<?php foreach ($data as $key => $value) { ?>
						<tr>
							...
							<td>
								<input type="text" name="cali_period_old[]" id="date_one<?=$value['asset_id'];?>" attr_id="<?=$value['asset_id'];?>" class="form-control" value="<?=$value['cali_period'];?>" readonly="true">
							</td>
							<td>
								<input type="date" name="cali_date_old[]" id="date_two<?=$value['asset_id'];?>" attr_id="<?=$value['asset_id'];?>" class="form-control" onchange="myDate1($(this))" value="<?=$value['cali_date'];?>">
							</td>
							<td>
								<input type="date" name="next_cali_date_old[]" id="date_three<?=$value['asset_id'];?>" attr_id="<?=$value['asset_id'];?>" class="form-control" onchange="myDate2($(this))" value="<?=$value['next_cali_date'];?>">
							</td>
							<td>
							<?php $array = array('','ไม่ผ่าน','ผ่าน'); ?>
							<select class="form-control status" name="cali_check_old[]">
							...
							<td>
								<input type="text" name="mana_asset_old[]" class="form-control" value="<?=$value['mana_asset']?>" readonly="true">
							</td>
							<td>
							<div class="input-group">
	...
	$('#save').click(function() {
		...
		if (count_tr > 0) {
			...
			$('.status').each( function(index, el) {
				...
				if($(el).val() == 0){
					status = false;
					$(el).attr('class', 'form-control status border-lg border-danger');
					return false;
				}else{
					$(el).attr('class', 'form-control status border-lg border-success');
				}
			});
			// alert(status);
			if (status) {
